...
int main()
{
	...
	// 블로킹(지금까지 했던 방식)
	// accept, connect, send, recv 등등
	// 접속한 클라가 있을때, 서버 접속 성공했을 때, 요청한 데이터를 송신 버퍼에 복사했을 때,
	// 수신 버퍼에 도착한 데이터가 있고, 이를 유저레벨 버퍼에 복사했을 때
	...
	// 주의사항
	// WSAEventSelect 함수를 호출하면 해당 소켓은 자동으로 논블로킹 모드로 전환
	...
	// ex) FD_READ 이벤트 떴으면 recv() 호출해야 하고, 안하면 FD_READ 두번 다시 발생 X
